...
wxImage FilterFaceswap::ApplyFilter(wxImage img)
{
	...
	// Clean aligned crop
	int tempSizeX = faces[0].x2 - faces[0].x1;
	int tempSizeY = faces[0].y2 - faces[0].y1;

	bBox bBoxTemp;
	bBoxTemp.x1 = faces[0].x1 - (tempSizeX / 2);
	bBoxTemp.y1 = faces[0].y1 - (tempSizeY / 2);
	bBoxTemp.x2 = faces[0].x2 + (tempSizeX / 2);
	bBoxTemp.y2 = faces[0].y2 + (tempSizeY / 2);

	cv::Mat processed = CropFace(bBoxTemp, mat);

	double dy = faces[0].bY - faces[0].aY;
	double dx = faces[0].bX - faces[0].aX;
	...
	if (arrFacesSource.empty()) return img;
	// Swap
	cv::Mat swap = Inswapper128(processed, arrFacesSource[1].embedding);
	cv::resize(swap, swap, cv::Size(faces[0].x2 - faces[0].x1, faces[0].y2 - faces[0].y1));
	...
	cv::Rect dst_roi(faces[0].x1, faces[0].y1, swap_bgra.cols, swap_bgra.rows);
...
void FilterFaceswap::BtnFindFaces(wxCommandEvent& event)
{
	...
	for (int i = 0; i < bBoxes.size(); i++)
	{
		...
		face.crop = crop;
		arrFacesTarget.push_back(face);

		wxImage thumb = ImgUtils::wxFromCv(crop);

		thumb = ImgUtils::Resize_Fit(thumb, 36, 36);

		wxButton* btn = new wxButton(scrollWindowFacesTarget, wxID_ANY, "", wxDefaultPosition, wxSize(40, 40));
...
void FilterFaceswap::BtnLoadFaces(wxCommandEvent& event)
{
	...
	for (int i = 0; i < files.GetCount(); i++)
	{
		...
		for (int j = 0; j < bBoxes.size(); j++)
		{
			...
			face faceTemp;
			faceTemp.cx = bBoxes[j].x1;
			faceTemp.cy = bBoxes[j].y1;
			faceTemp.sx = bBoxes[j].x2;
			faceTemp.sy = bBoxes[j].y2;
			...
			wxButton* btn = new wxButton(scrollWindowFacesSource, wxID_ANY, "", wxDefaultPosition, wxSize(40, 40));
			btn->SetBitmap(wxBitmap(thumb));
			btn->SetClientData(storedFace);

			btn->Bind(wxEVT_BUTTON, [this, storedFace](wxCommandEvent& event) {
				if (btnSelectedTarget == nullptr) return;

				btnSelectedTarget->SetClientData(storedFace);
				event.Skip();
				});
...
std::vector<FilterFaceswap::bBox> FilterFaceswap::Det10g(cv::Mat input)
{
	...
	SaveDetections(arr_anchorBoxes.size(), 0.4, 8);
	...
	SaveDetections(arr_anchorBoxes.size(), 0.6, 16);
	...
	SaveDetections(arr_anchorBoxes.size(), 0.8, 32);
